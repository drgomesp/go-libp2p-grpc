...
var log = golog.Logger("route_guide_client")

// printFeature gets the feature for the given point.
func printFeature(client pb.RouteGuideClient, point *pb.Point) {
	log.Infof("Getting feature for point (%d, %d)", point.Latitude, point.Longitude)
...
func printFeatures(client pb.RouteGuideClient, rect *pb.Rectangle) {
	log.Infof("Looking for features within %v", rect)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func main() {
	...
	log.Infow("Will get remote node info", "target", *serverAddr)
