...
var log = golog.Logger("route_guide_server")
...
func newRouteGuideServer(host host.Host) *routeGuideServer {
	s := &routeGuideServer{routeNotes: make(map[string][]*pb.RouteNote), host: host}
...
func main() {
	...
	m4, err := multiaddr.NewMultiaddr(fmt.Sprintf("/ip4/127.0.0.1/tcp/%d", *port))
	check(err)
	...
	log.Infof("I am %s", fullAddr)
	srv, err := libp2pgrpc.NewGrpcServer(ctx, ha, opts...)
	check(err)
	pb.RegisterRouteGuideServer(srv, newRouteGuideServer(ha))
